...
const { connect } = onSuccess(({ context: { getFromIndex } }: {context: {getFromIndex: (f: string, t: string) => void}}) => getFromIndex('field', 'theme'));
...
watch(settings, () => {
    isDark.value = (settings.value as Record<string, any>).value === 'dark';
});
